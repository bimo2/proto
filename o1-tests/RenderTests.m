...
- (void)test_collect_scroll {
    ...
    screen_set_cell(screen, 0, 0, 'A', NULL);
    render_collect_ops(&ops, screen, &count);
    XCTAssertEqual(count, 1);
    XCTAssertEqual(ops[0].op, RENDER_OP_SPAN);
    XCTAssertEqual(ops[0].span.row, 0);
    XCTAssertEqual(ops[0].span.column, 0);
    XCTAssertEqual(ops[0].span.width, 1);
    XCTAssertEqual(ops[0].span.cells[0].codepoint, 'A');

    render_clear_ops(ops, count);
    screen_set_cell(screen, 0, 0, 'B', NULL);
    ...
    XCTAssertEqual(ops[1].op, RENDER_OP_SPAN);
    XCTAssertEqual(ops[1].span.row, 0);
    XCTAssertEqual(ops[1].span.column, 0);
    XCTAssertEqual(ops[1].span.width, 5);
    XCTAssertEqual(ops[1].span.cells[0].codepoint, 'B');
    ...
    XCTAssertEqual(ops[0].scroll.delta, -1);
    XCTAssertEqual(ops[1].op, RENDER_OP_SPAN);
    XCTAssertEqual(ops[1].span.row, 4);
    XCTAssertEqual(ops[1].span.column, 0);
    XCTAssertEqual(ops[1].span.width, 5);

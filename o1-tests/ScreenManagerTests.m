...
@interface ScreenManagerTests : XCTestCase
...
@implementation ScreenManagerTests
...
- (void)test_update_text {
    ...
    screen_manager_update(manager, &repeat);
...
- (void)test_update_tabs {
    ...
    screen_manager_update(manager, &tab_set);
    ...
    screen_manager_update(manager, &tab);
    ...
    screen_manager_update(manager, &clear);
    screen_move_cursor_absolute(screen, 1, 1);
    screen_manager_update(manager, &tab);
...
- (void)test_update_cursor {
    screen_manager_t *manager = init_screen_manager();
    screen_manager_set_grid(manager, 5, 5);
    screen_t *screen = screen_manager_current_screen(manager);
    ...
    screen_manager_update(manager, &save);
    screen_move_cursor_absolute(screen, 1, 1);
    ...
    screen_manager_update(manager, &restore);
    ...
    screen_manager_update(manager, &position);
    ...
    screen_manager_update(manager, &up);
    ...
    screen_manager_update(manager, &back);
    XCTAssertEqual(screen_cursor(screen)->row, 0);
    XCTAssertEqual(screen_cursor(screen)->column, 2);
    ...
    screen_manager_update(manager, &next);
    XCTAssertEqual(screen_cursor(screen)->row, 2);
    ...
    screen_manager_update(manager, &previous);
    XCTAssertEqual(screen_cursor(screen)->row, 1);
    XCTAssertEqual(screen_cursor(screen)->column, 0);
    ...
    screen_manager_update(manager, &horizontal);
    XCTAssertEqual(screen_cursor(screen)->row, 1);
    XCTAssertEqual(screen_cursor(screen)->column, 2);
...
- (void)test_update_display {
    ...
    screen_manager_update(manager, &line);
    ...
    screen_manager_update(manager, &display);
    ...
    screen_manager_update(manager, &dec_display);
    ...
    screen_manager_update(manager, &dec_line);
...
- (void)test_update_scroll {
    ...
    screen_manager_update(manager, &down);
    XCTAssertEqual(screen_cell(screen, 0, 0)->codepoint, ' ');
    ...
    screen_manager_update(manager, &index);
    XCTAssertEqual(screen_cell(screen, 0, 0)->codepoint, '2');
    ...
    screen_manager_update(manager, &reverse_index);
    XCTAssertEqual(screen_cell(screen, 0, 0)->codepoint, ' ');
    XCTAssertEqual(screen_cell(screen, 1, 0)->codepoint, '2');
    ...
    screen_manager_update(manager, &margin);
...
- (void)test_update_mode {
    ...
    screen_manager_set_mouse_callback(manager, test_mouse_callback, (__bridge void *)weakSelf);
    ...
    screen_manager_update(manager, &insert);
    ...
    screen_manager_update(manager, &dec_set);
    ...
    screen_manager_update(manager, &dec_set);
    XCTAssertTrue(screen_manager_cursor_keys(manager));

    dec_set.csi.dec_mode = ANSI_DEC_MODE_MOUSE_ALL;
    screen_manager_update(manager, &dec_set);
    XCTAssertEqual(screen_manager_mouse_mode(manager), SCREEN_MANAGER_MOUSE_ALL);
    XCTAssertTrue(self.isMouseEnabled);

    dec_set.csi.dec_mode = ANSI_DEC_MODE_FOCUS_REPORTING;
    screen_manager_update(manager, &dec_set);
    XCTAssertTrue(screen_manager_focus_reporting(manager));

    dec_set.csi.dec_mode = ANSI_DEC_MODE_MOUSE_SGR;
    screen_manager_update(manager, &dec_set);
    XCTAssertTrue(screen_manager_mouse_sgr(manager));

    dec_set.csi.dec_mode = ANSI_DEC_MODE_BRACKETED_PASTE;
    screen_manager_update(manager, &dec_set);
    XCTAssertTrue(screen_manager_bracketed_paste(manager));
    ...
    screen_manager_update(manager, &dec_set);
    XCTAssertTrue(screen_manager_current_screen(manager) != screen);
    ...
    screen_manager_update(manager, &dec_reset);
    XCTAssertTrue(screen_manager_current_screen(manager) == screen);
    XCTAssertEqual(screen_cursor(screen)->row, 1);
    XCTAssertEqual(screen_cursor(screen)->column, 2);
    ...
    screen_manager_update(manager, &dec_reset);
    XCTAssertEqual(screen_manager_mouse_mode(manager), SCREEN_MANAGER_MOUSE_NONE);
    XCTAssertFalse(self.isMouseEnabled);

    dec_reset.csi.dec_mode = ANSI_DEC_MODE_FOCUS_REPORTING;
    screen_manager_update(manager, &dec_reset);
    XCTAssertFalse(screen_manager_focus_reporting(manager));

    dec_reset.csi.dec_mode = ANSI_DEC_MODE_MOUSE_SGR;
    screen_manager_update(manager, &dec_reset);
    XCTAssertFalse(screen_manager_mouse_sgr(manager));

    dec_reset.csi.dec_mode = ANSI_DEC_MODE_BRACKETED_PASTE;
    screen_manager_update(manager, &dec_reset);
    XCTAssertFalse(screen_manager_bracketed_paste(manager));

    dec_reset.csi.dec_mode = ANSI_DEC_MODE_CURSOR_KEYS;
    screen_manager_update(manager, &dec_reset);
    XCTAssertFalse(screen_manager_cursor_keys(manager));
...
- (void)test_update_device {
    ...
    screen_manager_set_response_callback(manager, test_response_callback, (__bridge void *)weakSelf);
    ...
    screen_manager_update(manager, &status);
    ...
    screen_manager_update(manager, &attributes);
...
- (void)test_update_hyperlink {
    ...
    screen_manager_update(manager, &hyperlink);
...
- (void)test_update_bell {
    ...
    screen_manager_set_bell_callback(manager, test_bell_callback, (__bridge void *)weakSelf);

    ansi_t bell = {
        .event = ANSI_EVENT_BELL,
    };

    screen_manager_update(manager, &bell);
...
static void test_response_callback(void *user_data, const char *response) {
    ScreenManagerTests *self = (__bridge ScreenManagerTests *)user_data;

    if (!self) return;

    __strong ScreenManagerTests *strongSelf = self;

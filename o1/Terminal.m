...
- (void)writeHardware {
    ...
    id cpu = hardware[@"number_processors"];
    NSInteger cores = 0;

    if ([cpu isKindOfClass:NSString.class]) {
        NSScanner *scanner = [NSScanner scannerWithString:(NSString *)cpu];
    ...
    } else if ([cpu isKindOfClass:NSNumber.class]) {
        cores = [(NSNumber *)cpu integerValue];
    }
